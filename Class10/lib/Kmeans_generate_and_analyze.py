...
#from matplotlib.path import arc
...
def make_scatter(df,centers,centroids,_figsize=[8,8]):
    _cmap = cm.get_cmap('Set1') # Colour map (there are many others)
    ...
    axes.scatter(centers[:, 0], centers[:, 1], c='g', s=40,label='True Center');
    axes.scatter(centroids[:, 0], centroids[:, 1],marker='v', c='y', s=100,label='centroid');
    df.plot.scatter(0,1,c='predicted_label',s=40,cmap=_cmap,figsize=_figsize,\
                    colorbar=False,marker=6,ax=axes,label='cluster_id');
    df.plot.scatter(0,1,c='label_error',s=40,cmap=binary_cmap,\
                   figsize=_figsize,marker='X',colorbar=False,ax=axes,label='incorrect');
    axes.scatter(centers[:, 0], centers[:, 1], c='g', s=40,label='true center');
    axes.scatter(centroids[:, 0], centroids[:, 1],marker='v', c='y', s=100,label='estimated centers');
    legend()
